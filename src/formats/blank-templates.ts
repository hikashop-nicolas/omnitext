...
// fflate is imported lazily inside the docx/odt builders so this eagerly-loaded module
// (its exported format lists feed the New dialog) does not pull fflate into the base bundle.
...
async function blankDocx(paper: Paper, orient: Orientation, direction: Direction): Promise<Uint8Array> {
...
async function blankOdt(paper: Paper, orient: Orientation, direction: Direction): Promise<Uint8Array> {
  const { strToU8, zipSync } = await import("fflate");
  return zipSync({
