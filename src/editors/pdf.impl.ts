...
class PdfInstance implements EditorInstance {
  ...
  /** The PDF itself, so the printer gets the document rather than the canvases. */
  async printableFile(): Promise<{ bytes: Uint8Array; mime: string }> {
    return { bytes: await this.getBytes(), mime: "application/pdf" };
  }
