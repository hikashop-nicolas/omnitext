...
const ja: Dict = {
  app: {
    ...
    themeLight: "ライト",
    themeDark: "ダーク",
    aiDownloads: "AI 機能のダウンロード",
    aiAsk: "最初に確認する",
    aiAllow: "許可",
    aiDeny: "許可しない",
    aiHint: "文字認識、翻訳、文章作成アシスト、文字起こしは初回利用時にモデルをダウンロードします。ファイルが送信されることはありません。",
    lineEndingMixed: "混在",
    yourName: "名前",
  ...
  notify: {
    ...
    archiveNotWritable: "Omnitext は {name} を読めますが書き戻せないため、このファイルをその中に保存できません。ダウンロードで別のファイルとして保存してください。",
  ...
  consent: {
    title: "ダウンロードが必要です",
    body: "{feature}には {hosts} から {what} のダウンロードが必要です（初回のみ）。その後はオフラインで動作し、ファイルがこの端末の外に出ることはありません。",
    about: "約",
    mb: "{n} MB",
    gb: "{n} GB",
    note: "この回答はすべての AI 機能に適用されます。設定で変更できます。",
    allow: "許可する",
    deny: "許可しない",
    turnedOff: "AI 機能のダウンロードはオフになっています。設定で許可できます。",
    feature: {
      ocr: "文字認識",
      translate: "翻訳",
      generate: "文章作成アシスト",
  ...
  tree: {
    ...
    kindList: "リスト [ ]",
    kindObject: "オブジェクト { }",
    kindLocked: "先に項目を削除してください。変換するとデータが失われます。",
  },
  binary: {
    download: "ダウンロード",
    openAsText: "テキストとして開く",
    looksText: "テキスト以外のバイトを少し含むテキストのようです。テキストとして開くと読めます。",
    ...
    askHint: "どのようなファイルか教えていただければ、Omnitext で開けるようになるかもしれません。ボタンを押すと、私たちのフィードバック管理システムのページが、ファイルの種類を入力済みの状態で開きます。書き込むには無料のアカウントが必要です。ファイルの内容は一切送信されません。",
