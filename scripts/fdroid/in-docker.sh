...
exec docker run --rm -v "$ROOT:/repo" -w /repo debian:trixie sh -c '
  ...
  apt-get -qq install -y --no-install-recommends git python3 make curl ca-certificates xz-utils bzip2 nodejs libdigest-sha3-perl libatomic1 unzip pkg-config patch gcc libc6-dev perl cmake ragel libtool libtool-bin itstool python3-ply gettext autopoint automake autoconf m4 gperf licensecheck gawk >/dev/null &&
